...
def target_selection_command_parser(parser):
	...
	parser.add_argument('--ra_targets', help='Select all items in remaining sequence as targets.', action='store_true')
...
def get_target_selection(args):
	return SelectTargets(n_targets=args.n_targets, shuffle=args.shuffle_targets, bias=args.target_bias, determinist_test=(not args.rand_test_target), ra_targets = args.ra_targets)
	

class SelectTargets(object):
	def __init__(self, n_targets=1, shuffle=False, bias=-1, determinist_test=True, ra_targets=False):
		super(SelectTargets, self).__init__()
		self.n_targets = n_targets
		self.shuffle = shuffle
		self.bias = bias
		self.determinist_test = determinist_test
		self.ra_targets = ra_targets
	...
	def __call__(self, remaining_sequence, test=False):
		...
		if self.ra_targets:
			return remaining_sequence[:len(remaining_sequence)]
		else:
